...
food <- read.delim(file = "data/processed_food/dhydrt.txt", row.names = 1)
...
# Goal is to sum sets of three days and label with the lsat day's date so they can be matched to microbiome samples
# triplets of food days correspond to microbiome days (0,1,2 = 3; 1,2,3 = 4; 2,3,4 = 5, etc.)
...
for (i in 1:length(unique(map$UserName))) {
  ...
  mynames <- submap$X.SampleID[3:17]
...
write.table(master3day, file = "data/processed_food/master3daydiet.txt", sep = "\t", quote = F, row.names = F, col.names = T)
