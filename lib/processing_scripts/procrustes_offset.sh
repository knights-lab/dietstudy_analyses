...
cd /Users/abby/Documents/Projects/dietstudy/analysis/procrustes/data_username_offset
...
for i in *_food_beta;
do transform_coordinate_matrices.py -i $i/${i//_beta}_pcoa.txt,${i//_food_beta}_tax_beta/${i//_food_beta}_tax_pcoa.txt -o ../results_username_offset/${i//_food_beta}_procrustes_results/ -r 999;
done

# move directories
cd ../results_username_offset

# make the plots
for i in *_procrustes_results;
do make_emperor.py -c -i $i/ -o $i/plots/ -m ../data_username_offset/${i//_procrustes_results}_map.txt --add_unique_columns --custom_axes StudyDayNo;
done
