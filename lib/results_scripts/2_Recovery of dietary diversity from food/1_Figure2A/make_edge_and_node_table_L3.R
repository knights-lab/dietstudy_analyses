...
food <- read.table(file ="analysis/procrustes/data/food_smry.txt", sep = "\t", header = T, comment = "", row.names = "taxonomy")
...
### make edge table ###
food <- rownames_to_column(food, "L3")
food <- melt(food)
food <- food %>% select(variable, L3, value)
food$L1 <- food$L3
...
write.table(edge.table, "analysis/food_network/food.edge.table.txt", sep = "\t", quote = F, row.names = F, col.names = T)
...
write.table(node.table, "analysis/food_network/food.node.table.txt", sep = "\t", quote = F, row.names = F, col.names = T)
