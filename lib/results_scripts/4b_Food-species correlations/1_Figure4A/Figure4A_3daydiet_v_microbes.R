...
load(file = "../../food_v_microbes_per_person_3day.RData")
...
names_repeated<- subset(names_repeated, names_repeated$Freq >1)
...
ggsave("../../../output/Figure4/Figure4A_3daydietpersonalizedcor.pdf",g, device = "pdf", width = 5, height = 9.5, dpi = 500)
...
export <- export[order(export$Species, export$Food),]
...
export$fdr_pval <- round(export$fdr_pval, 2)
export$ID <- gsub("MCTs", "", export$ID)



write.table(export, file = "../../../output/TableS1/Table S1.txt",
            sep = "\t",
            col.names = T,
            row.names = F,
            quote = F)
...
sigs.person <- sigs.person[-c(4,23)]
length(sigs.person)
table(sigs.person>1)
mean(sigs.person)
sd(sigs.person)       
