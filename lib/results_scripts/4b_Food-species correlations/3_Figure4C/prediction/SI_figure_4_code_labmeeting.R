...
### Predictors ####
# ## microbiome pricipal coordinates for today (Mb.Axis 1 - 3)
# ## diet principal coordinates for the 3-days prior to today (Dt.Axis 1 - 3)
...
"test.diet.impact" <- function(){
  ...
  for (k in 1:length(dat)) {
    ...
    for (i in seq_along(tomorrow.species)) {
      ...
      # TODO: scramble once for the model and once for the test?
      # TODO: train on scrambled and predict with real - noodle
      
      # now predict using the model for residuals trained on normal diet
      # but pass in scrambled diet features 
      pred.scramble.resid <- predict(dt.mb.model, scramble.dtA.feats)
